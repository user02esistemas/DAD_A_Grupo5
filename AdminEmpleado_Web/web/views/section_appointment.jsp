...
<section id="appointment" class="section">
                    ...
                    <div class="form-group">
                        ...
                        <div style="display: flex; gap: 10px;">
                            <select class="form-input" id="serviceType" onchange="calculateCost()" style="flex: 2;">
                                <option value="" data-price="0">Cargando servicios...</option>
                            </select>
    ...
    <script>
    (function(){
        fetch('ServiciosServlet').then(function(r){ return r.json(); }).then(function(services){
            var sel = document.getElementById('serviceType');
            if (!sel) return;
            sel.innerHTML = '<option value="" data-price="0">Seleccionar...</option>';
            services.forEach(function(s){
                var opt = document.createElement('option');
                opt.value = s.id;
                opt.setAttribute('data-price', s.precio);
                opt.innerText = s.nombre + ' (S/ ' + s.precio + ')';
                sel.appendChild(opt);
            });
        }).catch(function(){
            var sel = document.getElementById('serviceType');
            if (sel) {
                sel.innerHTML = '<option value="" data-price="0">Seleccionar...</option>' +
                    ...
                    '<option value="3" data-price="40">Restauraci\u00f3n (S/ 40)</option>';
            }
        });
    })();
    </script>
